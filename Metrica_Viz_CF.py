...
def plot_frame( hometeam, awayteam, figax=None, team_colors=('r','b'), field_dimen = (100.7,68.6), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate=False ):
...
def save_match_clip(hometeam,awayteam, fpath, fname='clip_test', figax=None, frames_per_second=25, team_colors=('r','b'), field_dimen =(100.7,68.6), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7,annotate=True,balltrack=False):
    ...
    """
    if 'ffmeg' in animation.writers.list():
            ...
            for i in index:
                ...
                for team,color in zip( [hometeam.loc[i],awayteam.loc[i]], team_colors) :
                    ...
                    if annotate:
                      #if i%25==0:
                      objs=[]
                      objs.append([ax.text( round(team[x],1)+0.5, round(team[y],1)+0.5, x.split('_')[1], fontsize=10, color=color  ) for x,y in zip(x_columns,y_columns) if not ( np.isnan(team[x]) or np.isnan(team[y]) ) ])
                      figobjs.append(objs)
                # plot ball
                ...
                # plot frame number
                frame=str(int(team['frameIdx']))
                objs = ax.text(-36.5,field_dimen[1]/2.+1., frame, fontsize=14 )
                figobjs.append(objs)

                if balltrack:
                    # plot x, y coord of ball
                    objs = ax.text( team['ball_x']-1.15, team['ball_y']-1.15, "({},{})".format(str(round(team['ball_x'],1)),str(round(team['ball_y'],1))))
                    figobjs.append(objs)
                ...
                for figobj in figobjs:
                    if isinstance(figobj, list):
                        for deep_state in figobj[0]:
                          deep_state.remove()
                        del figobj
                    elif isinstance(figobj, tuple):
                        figobj[1].remove()
                    else:
                        figobj.remove()
...
def plot_pitchcontrol_for_frame( frame, tracking_home, tracking_away, PPCF, alpha = 0.7, include_player_velocities=False, annotate=False, field_dimen = (100.7,68.6)):
    ...
    pass_frame = frame
    pass_team = tracking_home.loc[frame].Team
...
def plot_events( events, figax=None, field_dimen = (100.7,68.6), indicators = ['Marker','Arrow'], color='r', marker_style = 'o', alpha = 0.5, annotate=False, adjust=False):
    """ plot_events( events )
    ...
    -----------
        ...
        adjust: plots events based on ball position and original event vector (direction and magnitude)
...
def plot_pitchcontrol_for_event( event_id, events,  tracking_home, tracking_away, PPCF, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (100.7,68.6)):
    """ plot_pitchcontrol_for_event( event_id, events,  tracking_home, tracking_away, PPCF )
    ...
    fig,ax = plot_pitch(field_color='white', field_dimen = field_dimen)
    plot_frame( tracking_home.loc[pass_frame], tracking_away.loc[pass_frame], figax=(fig,ax), PlayerAlpha=alpha, include_player_velocities=include_player_velocities, annotate=annotate )
    plot_events( events.loc[event_id:event_id], figax = (fig,ax), indicators = ['Marker','Arrow'], annotate=False, color= 'k', alpha=1, adjust=True )
...
def plot_EPV_for_event( event_id, events, tracking_home, tracking_away, PPCF, EPV, alpha = 0.7, include_player_velocities=True, annotate=False, autoscale=0.1, contours=False, field_dimen = (100.7,68.6)):
...
def plot_EPV(EPV,field_dimen=(100.7,68.6),attack_direction=1):
