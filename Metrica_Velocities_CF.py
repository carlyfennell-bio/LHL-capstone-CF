...
def calc_player_velocities(team, smoothing=True, filter_='Savitzky-Golay', window=7, polyorder=1, maxspeed = 12):
    ...
    -----------
        ...
        the 'Savitzky-Golay' filter is a part of the scipy.signal package - using a moving average. Can look at this in more detail.
        ...
        12 is used here because it's the speed that Usain Bolt can run at :) 
    ...
    # Get the player ids
    player_ids = np.unique( [ c[:-2] for c in team.columns if c[:4].lower() in ['home','away'] ] )
    #player_ids = np.unique( [ c[:-2] for c in team.columns if c[:4] in ['Home','Away'] ] ) #original
    # Calculate the timestep from one frame to the next. Should always be 0.04 within the same half
    dt = team['Time [s]'].diff()

    ###
    #The diff() function is used to first discrete difference of element. Calculates the difference 
    #of a Series element compared with another element in the Series (default is element in previous row). 
    ...
    for player in player_ids: # cycle through players individually
        ...
        if maxspeed>0:
            ...
            # this can occur when the player is obstructed from view from the camera - a common issue with motion
            # capture type of data
