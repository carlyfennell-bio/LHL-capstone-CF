...
def check_offsides( attacking_players, defending_players, ball_position, GK_numbers, verbose=False, tol=0.2):
    ...
    defending_GK_id = GK_numbers[1] if attacking_players[0].teamname.lower()=='home' else GK_numbers[0]
...
def generate_pitch_control_for_frame(frame, tracking,tracking_home, tracking_away, params, GK_numbers, field_dimen = (100.7,68.6), n_grid_cells_x = 50, offsides=True):
    ...
    ##ADJUSTMENT ## Adding ball speed 
    ball_speed= tracking.loc[frame]['ball_speed']
    #ball_speed=params['average_ball_speed'] 
    pass_frame = frame
    pass_team = tracking.loc[frame]['Team']
    ...
    for i in range( len(ygrid) ):
        for j in range( len(xgrid) ):
            ...
            PPCFa[i,j],PPCFd[i,j] = calculate_pitch_control_at_target(target_position, attacking_players, defending_players, ball_start_pos, params,ball_speed)
    # check probabilitiy sums within convergence
    checksum = np.sum( PPCFa + PPCFd ) / float(n_grid_cells_y*n_grid_cells_x ) 
    assert 1-checksum < params['model_converge_tol'], "Checksum failed: %1.3f" % (1-checksum)
    return PPCFa,xgrid,ygrid

def generate_pitch_control_for_event(event_id, events, tracking_home, tracking_away, params, GK_numbers, field_dimen = (100.7,68.6), n_grid_cells_x = 50, offsides=True):
    """ generate_pitch_control_for_event
    ...
    """
    # get the details of the event (frame, team in possession, ball_start_position)
    # MAJOR ADJUSTMENT_CF: set ball_start_position to the tracking ball location instead

    ##ADJUSTMENT ## Adding ball speed 
    ball_speed= events.loc[event_id]['ball_speed']
    #ball_speed=params['average_ball_speed'] 
...
def calculate_pitch_control_at_target(target_position, attacking_players, defending_players, ball_start_pos, params,ball_speed):
    ...
    if ball_start_pos is None or any(np.isnan(ball_start_pos)): # assume that ball is already at location
        ...
        ### SHOULD ADJUST THE BALL SPEED TO BE THE ACTUAL BALL SPEED NOT AVERAGE
