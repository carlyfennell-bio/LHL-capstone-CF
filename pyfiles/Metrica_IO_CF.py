...
def to_metric_coordinates(data,field_dimen=(107,68.6)):
    '''
    Convert positions from Metrica units to meters (with origin at centre circle)
    '''
    x_columns = [c for c in data.columns if c[-1].lower()=='x'] # finds any column ending in an "X"
    y_columns = [c for c in data.columns if c[-1].lower()=='y'] # finds any column ending in an "Y"
...
def to_single_playing_dir(home,away):
    '''
    Flip coordinates in second half so that each team always shoots in the same direction through the match. Try taking events out
    '''
...
def find_goalkeeper(team,frame):
    '''
    ...
    if x_columns[0][:4]=='home':
        GK_col = team.iloc[frame][x_columns]
        return GK_col[GK_col==np.nanmax(GK_col)].index[0].split('_')[1]
    elif x_columns[0][:4]=='away':
        GK_col = team.iloc[frame][x_columns]
        return GK_col[GK_col==np.nanmin(GK_col)].index[0].split('_')[1]
